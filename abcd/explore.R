...
m<-as.data.frame(tbl(db,sql("select v.id,Name,mot.value as motion,er.value as extrisk, mint,maxt,finaldur,Date,Time,ageatscan,Sex,Operator,seqno,ndcm,RT from mrinfo m join visit v on m.scanid=v.scanid left join motion mot on mot.id=v.id left join extrisk er on er.id=v.id")))

m$dt <- datetime(m$Date,m$Time)

smry <- 
  m %>% 
  group_by(id,Date,Operator,Sex,motion,ageatscan,extrisk) %>% 
  arrange(dt) %>%
  summarise(
